...
if start 
   ...
   % ask for new parameters. Default values
   if(isfield(params,'useLS')==0)  % Find if it is available
       params.useLS = 0;
   end
   
   if(isfield(params,'LSniter')==0)
       params.LSniter = 400;       % Default value for optimization iterations
   end
   
   if(isfield(params,'inicparamvalue')==0)
      params.inicparamvalue = 1;
   end
   
   if(isfield(params,'extendtrees')==0)
      params.extendtrees = 1;
   end

   if(isfield(params,'LSbest')==0)
      params.LSbest = 1;
   end
   
   if(isfield(params,'LSworst')==0)
      params.LSworst = 0;
   end

   if(isfield(params,'LSheuristic')==0)
      params.LSheuristic = 0;
   end
   
   if(isfield(params,'LSasc')==0)
      params.LSasc = 1;
   end

   if(isfield(params,'LSprob')==0)
      params.LSprob = 1;
   end
