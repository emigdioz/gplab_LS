...
if ~isempty(ind1.extended)
   if (params.useLS) && (ind1.extended) % Restricted crossover point when individual is extended
       x1=intrand(4,ind1.nodes-1); % 4 th node crossover
   else
...
if ~isempty(ind2.extended)
   if (params.useLS) && (ind2.extended) % Restricted crossover point when individual is extended
       x2=intrand(4,ind2.nodes-1);
   else
